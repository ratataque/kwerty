...
export function getConfigCategories(): {
    ...
    > = {
        macos: {
            label: "macOS",
            description: "AeroSpace, skhd, Karabiner, Sketchybar, Neovim",
        },
        "arch-linux": {
            label: "Arch Linux",
            description: "Hyprland, xremap, rofi, Neovim, quickshell",
        },
        windows: { label: "Windows", description: "GlazeWM, Zebar" },
        shared: {
            label: "Shared",
            description: "Cross-platform terminal, shell, and tool configs",
        },
    };
...
export function getReposByContributor(contributor: string): Repo[] {
    return readJson<Repo[]>("repos.json").filter(
        (r) => r.contributor === contributor,
    );
}
